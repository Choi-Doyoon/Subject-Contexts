...
public:
    ...
    void add_node(const T& value){
        ...
        adj_matrix.push_back(vector<int>(size-1,0));
    ...
    int get_weight(size_t source, size_t target){
        return adj_matrix[source][target];
    }
    ...
    vector<int> dijkstra(Graph<T> g, int start){
        int INT_MAX = 2147483647;
        vector<int> distance = vector<int>(g.get_size(),INT_MAX);
        vector<bool> is_visited = vector<bool>(g.get_size(),false);
        int current = start;
        distance[start]=0;
        ...
        for(int i=0;i<g.get_size();i++){
            ...
            for(int j=0;j<g.get_size();j++){
                if(g.get_weight(current, j)){
                    distance[j] = min(distance[j], distance[current] + g.get_weight(current, j));
                }
            }

            int min = INT_MAX;
            for(int j=0;j<g.get_size();j++){
                if(!is_visited[j] && distance[j]<min){
                    min=distance[j];
                    current = j;
                }
            }
...
int main(){
    ...
    cout<<endl;
    g.BFS(g,0);
    vector<int> distance = g.dijkstra(g,0);
    cout<<endl;
    cout<<"Dijkstra shortest path cost: ";
    for(int i=0;i<g.get_size();i++){
        cout<<distance[i]<<" ";
    }
    cout<<endl;
